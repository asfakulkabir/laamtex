...
<div class="max-w-xl bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800/50 p-8 rounded-2xl shadow-lg">
    ...
    <div class="mb-6">
        <h3 class="font-bold text-white text-lg">Shipping Zone Details</h3>
        <p class="text-sm text-slate-300 mt-1">Specify regional names, prices, and shipping durations.</p>
    </div>

    <form action="{{ route('admin.delivery-charges.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="number" step="0.01" min="0" id="charge" name="charge" value="{{ old('charge') }}" required
                   class="w-full bg-slate-800/50 border border-slate-700/50 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:bg-slate-800/50 transition-all text-slate-200 placeholder-slate-600"
        ...
        <div>
            ...
            <input type="text" id="estimated_days" name="estimated_days" value="{{ old('estimated_days') }}"
                   class="w-full bg-slate-800/50 border border-slate-700/50 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:bg-slate-800/50 transition-all text-slate-200 placeholder-slate-600"
